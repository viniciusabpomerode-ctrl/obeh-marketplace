...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (adminData.email !== ADMIN_EMAIL) return { statusCode: 403, body: JSON.stringify({ error: 'Acesso negado' }) }

    const { loja_id, email_destino, plano, meses } = body
    if (!loja_id) return { statusCode: 400, body: JSON.stringify({ error: 'ID da loja é obrigatório' }) }
    if (!email_destino) return { statusCode: 400, body: JSON.stringify({ error: 'Email do destinatário é obrigatório' }) }
    ...
    if (!lojaAdmin) return { statusCode: 404, body: JSON.stringify({ error: 'Loja não encontrada ou não é do admin' }) }
    ...
    if (!userDestino) return { statusCode: 404, body: JSON.stringify({ error: 'Usuário não encontrado com esse email' }) }
    ...
    if (lojaDestino) {
      ...
      return { statusCode: 200, body: JSON.stringify({ success: true, merged: true, loja_id: lojaDestino.id, mensagem: 'Loja mesclada! Produtos transferidos, branding preservado.' }) }
    } else {
      ...
      return { statusCode: 200, body: JSON.stringify({ success: true, merged: false, loja_id: lojaAdmin.id, mensagem: 'Loja transferida com sucesso!' }) }
    }
  } catch (err) {
